    ...
    <div className={`bg-[#141414] ${option==="Select"?"h-screen":"h-full"}`}>
      <div className="fixed z-10 pt-[14.5%] md:pt-[4.7%] w-full">
        <div className={`flex flex-row pt-4 md:pt-1 pl-10 md:pl-20 ${isScrolled?'bg-[#141414]':''}`}>
          ...
          <button className="bg-white flex w-32 sm:w-60 rounded-lg" onClick={handleOptionMenuClick}><h1 className="mx-auto my-auto text-xs sm:text-base">{option}</h1> <FaChevronDown className="DropDownIcon mr-1 my-2" style={{color:"blue"}}/></button>
        </div>
        {showOptionMenu && <div className={`fixed flex flex-wrap mt-2 ml-[6.5rem] text-black bg-white max-w-[15rem] sm:max-w-xs rounded-lg ${isScrolled?'bg-opacity-100':'bg-opacity-50'}`}>
          {Genres.map(genre => <Link className="m-1.5 hover:text-[#00ADB5] text-xs sm:text-base" to={"/body/genre/"+genre.id} key={genre.id} onClick={()=>{handleOptionClick(genre.name)}}>{genre.name}</Link>)}
        </div>}
      </div>
      <Outlet/>
    </div>
