    ...
    return (
        <>
            {showSearchBar && <div className="col-span-8 md:col-span-6 mx-auto w-2/3 md:w-1/2 flex items-center" onClick={handleSearch} onKeyDown={handleKeyDown}>
                <input type="text" placeholder="Search" className="w-full md:p-4 md:pl-1 pl-1 p-2 text-black h-6 rounded-md" onChange={(e)=>{handleSearchChange(e.target.value)}} value={searchInput}/>
                <div className="ml-2">
                    <span class="changecolor"> 
                        <FaSearch onmouseover={({ target }) => (target.style.color = "#00ADB5")}
                        onmouseout={({ target }) => (target.style.color = "#EEEEEE")} className="SearchIcon"/>
                    </span>
                </div>
            </div>}
        </>
